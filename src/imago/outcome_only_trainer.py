"""
Emulates trainer, but only trains the outcomes
"""
...
def train(model, output_dir,
    ...
    def validate(beta, step):
        ...
        for datum in tqdm_iter:
            if isinstance(datum, torch.Tensor):
                ...
                O = datum[0]
                Ts = [D.to(device) for D in
                      datum[1:]]  # Additional targets beyond reconstruction targets
            ...
            O = O.to(device)
            loss, named_losses = model_loss(model, O, Ts=Ts, beta=beta, outcome_only=True)
    ...
    for epoch in range(epochs):
        ...
        for datum in tqdm_iter:
            ...
            optimizer.zero_grad()
            O = O.to(device)
            loss, named_losses = model_loss(model, O, Ts=Ts,outcome_only=True)
            ...
            tqdm_iter.set_description("Epoch {} step={}, beta={:.5f}, loss={:.5f}, epoch loss={:5f}".format(
                epoch_num, step, 0, loss.item() / batch_size, epoch_loss))
            tb_sw.add_scalar("Loss", loss.item() / batch_size, global_step=step)
