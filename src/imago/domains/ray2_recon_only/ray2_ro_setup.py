...
from imago import IMAGO_ROOT, CAML_ROOT
...
DEFAULT_MODEL_FPATH = Path(IMAGO_ROOT, "models", "ReaverAssaultYear2_ReconOnly", "model.pt")
...
def load_model(model_path=DEFAULT_MODEL_FPATH, device="cpu", force_new=False):
    model = PVaeModel(mcbox_spec, latent2target_specs=[],
...
def compute_Odiff(O1, O2):
    ...
    O1 = mcbox_spec.make_onehot(O1)
    O2 = mcbox_spec.make_onehot(O2)
