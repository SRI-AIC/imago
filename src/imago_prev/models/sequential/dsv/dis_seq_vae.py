...
def init_weights(model):
    for m in model.modules():
        if isinstance(m, nn.BatchNorm1d) or isinstance(m, nn.BatchNorm2d):
            logging.info("{}: Initializing module as BatchNorm".format(m))
            nn.init.constant_(m.weight, 1)
            nn.init.constant_(m.bias, 1)
        elif isinstance(m, nn.Conv2d) or isinstance(m, nn.ConvTranspose2d) or isinstance(m, nn.Linear):
            logging.info("{}: Initializing module via Kaiming".format(m))
            nn.init.kaiming_normal_(m.weight, nonlinearity='relu')
        else:
            logging.info("{}: Default init".format(m))
...
class DynamicEncoder(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, latent_size, dropout=0):
        ...
        self.rnn = nn.RNN(input_size=hidden_size * 2, hidden_size=hidden_size,
                          batch_first=True)
        self.linear_mu = nn.Linear(hidden_size, latent_size)     # RNN single direction only
...
class PerceptualEncoder(nn.Module):
    ...
    def forward(self, X):
        ...
        X = X.view(-1, self.nc, self.h, self.w)  # Unroll all images into single batched format
        X = self.conv1(X)
        ...
        return X.view(-1, self.num_frames, self.x_dim)
...
class Decoder(nn.Module):
    ...
    def forward(self, X):
        if X.shape == 2:
            # Dealing with (batch_size, Z+F), primarily for rollout or singular prediction
            num_frames = 1
            rollout_mode = True
        else:
        ...
        if not(rollout_mode):
            ...
            X = X.view(-1, self.nc, self.h, self.w)
        return X

DSVResult = collections.namedtuple('DSVResult', ['Xhat', "F_mu", "F_logvar", "Z_mu", "Z_logvar", "Z", "F"])

class DisSeqVAE(nn.Module):
    ...
    def __init__(self,
                 num_frames=8,  # Number of fixed frames for model
        ...
        super(DisSeqVAE, self).__init__()
        self.num_frames, self.z_dim, self.f_dim = num_frames, z_dim, f_dim
        self.x_dim = x_dim
        ...
        # Perceptual encoder
        # TODO: Consider Resnet, since the architecture seems to imply a desire
        # to do some feature discovery after initial convolution
    ...
    def forward(self, O):
        ...
        """
        X = self.x_encoder(O)  # Encode obs frames individually
    ...
    def rollout(self, O):
        """
        Given a O_{1:T}, encodes Z_{1:T}, f, and then performs a rollout
        :param O:
        :return:
        """
...
def elbo_loss(X, r):
    ...
    :return:
    """
    mse = tfunc.mse_loss(X, r.Xhat, reduction='sum') # Expectation of data under q(x)
    ...
    return mse - (kld_F + kld_Z), mse, -1 * (kld_F + kld_Z)