#
# Rollout Semantic Frame Disentangled Sequential Autoencoder.
# Targets the 1-hot semantic frame representation used in SC2
#
import torch
...
def multinomial_loss_fn(y_hat, y_gold):
    """ Given a hypothesis matrix of the shape (batch, num_frames, TARGETS, width, height) and
    a target of (batch, width, height) with integer labels for TARGET offset, computes
    the softmax and NLL loss.
    """
    hyp = tfunc.log_softmax(y_hat, dim=2)
    loss = tfunc.nll_loss(hyp, y_gold)
    return loss
...
class RolloutSemDisSeqVAE(nn.Module):
    ...
    def __init__(self,
        ...
        super(RolloutSemDisSeqVAE, self).__init__()
...
SemRDSVLoss = collections.namedtuple('SemRDSVLoss', ['loss', 'elbo', 'kld', 'recon', 'dyn_mse'])
...
def loss_fn(X_full, r):
    ...
    recon = multinomial_loss_fn(X_full, r.X_hat, reduction='sum')
    ...
    return SemRDSVLoss(loss=elbo + dynamics_mse,
                    elbo=elbo,
                    kld=- (kld_F + kld_Z),
                    recon=recon,
